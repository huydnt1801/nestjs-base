...
import { ApiCauses } from '../../config/exception/api-causes';
...
export class BlockchainService {
    ...
    async getAddress(coin: string, params = {}) {
        ...
        try {
            ...
            return this._forwardError(e, `Could not create new address coin=${coin} params=${JSON.stringify(params)}`);
        }
    }
    ...
    async getBalance(systemSymbol: string, address: string) {
        ...
        try {
            ...
            return this._forwardError(e, `Could not get balance of coin=${systemSymbol} address=${address}`);
        }
    }
    ...
    async getCurrencyConfiguration(coin: string, contractAddress: string) {
        const api = await this.getApiEndpoint(coin);
        try {
            const response = await this.httpService.get(`${api}/currency_config/${contractAddress}`).toPromise();
            return response.data;
        } catch (e) {
    ...
    async validateAddress(coin: string, address: string) {
        const api = await this.getApiEndpoint(coin);
        try {
            const response = await this.httpService.get(`${api}/${coin}/address/${address}/validate`).toPromise();
            return response.data;
        } catch (e) {
    ...
    async isNeedTag(coin: string, address: string) {
        const api = await this.getApiEndpoint(coin);
        try {
            const response = await this.httpService.get(`${api}/${coin}/address/${address}/tag`).toPromise();
            return response.data;
        } catch (e) {
            return this._forwardError(e, `Could not validate coin=${coin} address=${address}`);
        }
    ...
    async getNormalizeAddress(coin: string, address: string) {
        const api = await this.getApiEndpoint(coin);
        try {
            const response = await this.httpService.get(`${api}/${coin}/address/${address}/normalized`).toPromise();
            return response.data;
        } catch (e) {
    ...
    async getEstimateFee(
        ...
        try {
            ...
            return this._forwardError(e, `Could not estimate fee for currency: ${coin} with params: ${params}.`);
        }
    }
